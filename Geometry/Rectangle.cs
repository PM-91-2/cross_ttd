...
namespace Geometry {
	public class Rectangle : IFigure {
		...
		public void Rotate(float angle) {
			...
			for (int i = 0; i < _points.Count; i++)
				_points[i] = new Vector2((rotateMatrix[0].X *( _points[i].X - _position.X) + rotateMatrix[0].Y * (_points[i].Y - _position.Y) + _position.X), 
										(rotateMatrix[1].X * (_points[i].X - _position.X) + rotateMatrix[1].Y * (_points[i].Y - _position.Y) + _position.Y));
		}
